...
public class Pixel extends PointSource{
	...
	private double sizeSquared = 4; // how many pixels the 'pixel' represents
	...
	public boolean isObstructed(Light light, ArrayList<GraphicalObject> op) { // Do we mix the light onto the pixel, or not
...
//		}
		...
		for (int i = 0; i < op.size(); i++) {
			...
			if (l.intersects(r) && op.get(i).isCastsShadows()) { // if the line from the light to the pixel is obstructed by a graphical object, then don't mix that light
	...
	public void calculateColor(Object[] l, ArrayList<GraphicalObject> sceneContents, Graphics2D g) {
		...
		for (int i = 0; i < lights.length; i++) {
			...
			if (distance <= lights[i].getDistance()) { // within distance & is in light path
				...
				if (!iosc && isObstructed(lights[i], sceneContents)) {
					alphaStage *= 0.25;
				}
